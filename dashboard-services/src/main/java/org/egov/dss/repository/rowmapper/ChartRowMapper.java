...
public class ChartRowMapper implements ResultSetExtractor<List<Chart>> {
	...
	public List<Chart> extractData(ResultSet rs) throws SQLException, DataAccessException {
		...
		while (rs.next()) {
			...
			Chart chartResponse = Chart.builder().name(rs.getString("name"))
					.value(rs.getBigDecimal("value")).build();
